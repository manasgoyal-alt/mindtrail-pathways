...
export function CheckInScreen() {
  const navigate = useNavigate(); const [step,setStep]=useState(0); const [answers,setAnswers]=useState<Record<string,string>>({}); const [note,setNote]=useState(""); const [saving,setSaving]=useState(false); const [error,setError]=useState(""); const done=step===steps.length; const current=steps[step];
  const finish=async()=>{setSaving(true);setError("");try{const mood=steps[0].options[Number(answers["mood"])-1]?.toLowerCase();if(!mood)throw new Error("Missing mood");await mindtrailService.saveCheckIn({mood:mood as "steady"|"low"|"tense"|"hopeful"|"overwhelmed",energy:Number(answers["energy"]),stress:Number(answers["stress"]),sleep:Number(answers["sleep"]),note});navigate({to:"/today"})}catch{setError("Your check-in could not be saved. Please try again.")}finally{setSaving(false)}};
  return <AppShell><div className="mx-auto flex min-h-screen max-w-3xl flex-col px-5 py-8 sm:px-8 lg:py-12">
    ...
    {error&&<p role="alert" className="mb-3 text-sm text-destructive">{error}</p>}<div className="flex items-center justify-between border-t pt-5"><Button variant="ghost" onClick={()=> done ? navigate({to:"/today"}) : setStep(step+1)}>Skip</Button><Button disabled={saving||(!done && current ? !answers[current.key] : false)} onClick={()=> done ? finish() : setStep(step+1)}>{done ? <><Check /> {saving?"Saving…":"Finish check-in"}</> : <>Continue <ArrowRight /></>}</Button></div>
  </div></AppShell>;
}