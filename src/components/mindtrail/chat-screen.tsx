...
export function ChatScreen({ threadId }: { threadId: string }) {
  ...
  const submit=async (message:PromptInputMessage)=>{ const text=message.text.trim(); if(!text||status!=="ready")return; const user: UIMessage={id:crypto.randomUUID(),role:"user",parts:[{type:"text",text}]}; setMessages((m)=>[...m,user]); setStatus("submitted"); await new Promise((resolve)=>setTimeout(resolve,900)); setMessages((m)=>[...m,{id:crypto.randomUUID(),role:"assistant",parts:[{type:"text",text:"That sounds like a lot to hold at once. We can slow it down together. Would it help to name the most pressing part, or choose one small thing you can leave for tomorrow?"}]}]); setStatus("ready"); };
